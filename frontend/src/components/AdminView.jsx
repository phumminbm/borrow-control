...
export default function AdminView({ customers, syncLogs, dark, analytics, custValues = {}, lang = "th" }) {
  ...
  const da = h2hA ? h2hData(h2hA) : null;
  const db2 = h2hB ? h2hData(h2hB) : null;

  const card = (children, extra={}) => ({
    background: dark?"#141414":"#fff",
    border:`0.5px solid ${dark?"#222":"rgba(0,0,0,0.1)"}`,
    borderRadius:10, padding:14, ...extra
  });
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 50px)"}}>
      ...
      <div style={{flexShrink:0}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
          <div style={{background:dark?"#1a1a1a":"var(--color-background-primary)",border:`1.5px solid ${dark?"#2a2a2a":"var(--color-border-secondary)"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#ddd":"#888",marginBottom:2}}>{t.totalCustomers}</div>
          ...
          <div style={{background:dark?"#2D1010":"#FCEBEB",border:`1.5px solid ${dark?"#7A2020":"#F09595"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#F09595":"#A32D2D",marginBottom:2,fontWeight:500}}>BLOCK</div>
          ...
          <div style={{background:dark?"#2D1E00":"#FAEEDA",border:`1.5px solid ${dark?"#7A5500":"#FAC775"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#FAC775":"#854F0B",marginBottom:2,fontWeight:500}}>WARNING</div>
          ...
          <div style={{background:dark?"#162010":"#EAF3DE",border:`1.5px solid ${dark?"#3A6014":"#C0DD97"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#C0DD97":"#3B6D11",marginBottom:2,fontWeight:500}}>NORMAL</div>
            <div style={{fontSize:20,fontWeight:600,color:dark?"#C0DD97":"#3B6D11"}}>{filteredNo}</div>
          </div>
          <div style={{background:dark?"#1a1a1a":"var(--color-background-primary)",border:`1.5px solid ${dark?"#2a2a2a":"var(--color-border-secondary)"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#ddd":"#888",marginBottom:2}}>{t.brActive}</div>
          ...
          <div style={{background:dark?"#1a1a1a":"var(--color-background-primary)",border:`1.5px solid ${dark?"#7A2020":"#F09595"}`,borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:dark?"#F09595":"#A32D2D",marginBottom:2,fontWeight:500}}>{t.totalValue}</div>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12}}>
        ...
        {/* Row 3: Top10 Value + Head-to-Head */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div style={card()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"22px 1fr 65px 80px 70px",gap:4,fontSize:10,fontWeight:500,color:"#555",marginBottom:6,paddingBottom:4,borderBottom:`0.5px solid ${dark?"#2a2a2a":"rgba(0,0,0,0.08)"}`}}>
            ...
            {[...filtered].sort((a,b)=>(custValues[b.cust_code]||0)-(custValues[a.cust_code]||0)).slice(0,10).map((c,i)=>{
              ...
              return (
                <div key={c.cust_code} style={{display:"grid",gridTemplateColumns:"22px 1fr 65px 80px 70px",gap:4,alignItems:"center",padding:"6px 0",borderBottom:`0.5px solid ${dark?"#1e1e1e":"rgba(0,0,0,0.05)"}`,fontSize:11,background:c.status==="BLOCK"?(dark?"#1e0e0e":"transparent"):c.status==="WARNING"?(dark?"#1e1600":"transparent"):"transparent"}}>
                  ...
                  <span style={{textAlign:"center"}}>
                    {c.status==="BLOCK"?<span style={{background:dark?"#3D1212":"#E24B4A",color:dark?"#F09595":"#fff",border:dark?"0.5px solid #7A2020":"none",borderRadius:4,padding:"1px 6px",fontSize:9,fontWeight:600}}>BLOCK</span>
                    :c.status==="WARNING"?<span style={{background:dark?"#3D2A00":"#EF9F27",color:dark?"#FAC775":"#fff",border:dark?"0.5px solid #7A5500":"none",borderRadius:4,padding:"1px 6px",fontSize:9,fontWeight:600}}>WARNING</span>
          ...
          <div style={card()}>
            ...
            {(!h2hA || !h2hB) ? (
              ...
              const maxVal = Math.max(da.total, db2.total, 1);
              return (
                <>
                  ...
                  {[
                    ["ลูกค้า",da.total,db2.total,Math.max(da.total,db2.total,1)],
                    ["BLOCK",da.block,db2.block,Math.max(da.block,db2.block,1)],
                    ["WARNING",da.warn,db2.warn,Math.max(da.warn,db2.warn,1)],
                    ["NORMAL",da.normal,db2.normal,Math.max(da.normal,db2.normal,1)],
                    ["วันค้างสูงสุด",da.maxDays,db2.maxDays,Math.max(da.maxDays,db2.maxDays,1)],
                  ].map(([label,v1,v2,max])=>(
            ...
            <thead>
              <tr style={{position:"sticky",top:0,zIndex:5,background:dark?"#1a1a1a":"#f9f9f7"}}>
                {["#",t.custCode,t.custName,t.team,"Sale",t.br,t.daysOverdue,t.status,t.updated].map((h,i)=>(
                  <th key={i} style={{padding:"8px 11px",textAlign:"left",fontSize:11,fontWeight:500,color:dark?"#ddd":"#888",
