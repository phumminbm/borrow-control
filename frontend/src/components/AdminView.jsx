...
function BarChart({ team, customers, dark }) {
  const ref = useRef();
  useEffect(() => {
    ...
    stats.forEach((s,i) => {
      ...
      if(total>0){ctx.fillStyle=dark?"#aaa":"#666";ctx.font="bold 9px sans-serif";ctx.fillText(total,x+barW/2,pad.top+chartH-(total/maxVal)*chartH-4);}
    });
  }, [team, customers, dark]);
  return <canvas ref={ref} style={{width:"100%",height:160,display:"block"}}/>;
}

export default function AdminView({ customers, syncLogs, dark }) {
  ...
  const filtered = customersWithTeam.filter(c =>
    (!search || c.customer_name.toLowerCase().includes(search.toLowerCase()) || String(c.cust_code).toLowerCase().includes(search.toLowerCase())) &&
  ...
  const bdr   = dark ? "#2a2a2a" : "rgba(0,0,0,0.1)";
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {[["ลูกค้าทั้งหมด",customersWithTeam.length,null],["BLOCK",totalBl,"#E24B4A"],["WARNING",totalWa,"#EF9F27"],["NORMAL",totalNo,"#639922"],["BR active",totalBR,null]].map(([l,v,c])=>(
          <div key={l} style={{background:card,borderRadius:8,padding:"9px 14px",flex:1,minWidth:80}}>
            <div style={{fontSize:11,color:"#888",marginBottom:2}}>{l}</div>
            <div style={{fontSize:19,fontWeight:600,color:c||(dark?"#eee":"#1a1a1a")}}>{typeof v==="number"?v.toLocaleString():v}</div>
          </div>
        ))}
      </div>
      ...
      {/* Sync status */}
      {lastSync && (
        <div style={{background:dark?"#1a1a1a":"#f9f9f7",border:`0.5px solid ${bdr}`,borderRadius:8,padding:"8px 14px",marginBottom:14,fontSize:11,color:"#888",display:"flex",flexWrap:"wrap",gap:12,alignItems:"center"}}>
          ...
          <span style={{color:lastSync.errors>0?"#E24B4A":"#639922"}}>{lastSync.errors} error</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1.7fr)",gap:12,marginBottom:14}}>
        <div style={{background:panel,border:`0.5px solid ${bdr}`,borderRadius:10,padding:14,display:"flex",gap:14,alignItems:"center"}}>
          <Donut bl={pieData.bl} wa={pieData.wa} no={pieData.no}/>
          <div style={{flex:1,fontSize:12}}>
            <div style={{fontWeight:600,marginBottom:8,color:dark?"#aaa":"#555"}}>{selTeam||"ภาพรวม"}</div>
            {[["BLOCK",pieData.bl,"#E24B4A"],["WARNING",pieData.wa,"#EF9F27"],["NORMAL",pieData.no,"#639922"]].map(([l,v,c])=>(
              <div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                <span style={{display:"flex",alignItems:"center",gap:5}}>
                  <span style={{width:8,height:8,borderRadius:2,background:c}}/><span style={{color:dark?"#aaa":"#555"}}>{l}</span>
                </span>
                <span style={{fontWeight:600,color:c}}>{v}</span>
                <span style={{color:dark?"#555":"#ccc",fontSize:10,minWidth:36,textAlign:"right"}}>{pieData.total?Math.round(v/pieData.total*100):0}%</span>
              </div>
            ))}
            <div style={{borderTop:`0.5px solid ${bdr}`,paddingTop:6,display:"flex",justifyContent:"space-between"}}>
              <span style={{color:"#aaa",fontSize:11}}>รวม</span>
              <strong style={{fontSize:11,color:txt}}>{pieData.total} ราย</strong>
            </div>
          </div>
        </div>

        <div style={{background:panel,border:`0.5px solid ${bdr}`,borderRadius:10,padding:14}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
            {Object.keys(TEAMS).map(t=>(
              <button key={t} onClick={()=>{setSelTeam(selTeam===t?null:t);setTeamFilter(selTeam===t?"":t);setSaleFilter("");}} style={{
                ...
                background:selTeam===t?TEAM_COLORS[t]:(dark?"#222":"#fff"),
                color:selTeam===t?"#fff":TEAM_COLORS[t],
                borderColor:TEAM_COLORS[t],transition:"all .15s",
              }}>{t}</button>
            ))}
            {selTeam&&<button onClick={()=>{setSelTeam(null);setTeamFilter("");setSaleFilter("");}} style={{padding:"4px 11px",fontSize:11,borderRadius:20,cursor:"pointer",border:`0.5px solid ${inpBdr}`,background:dark?"#2a2a2a":"#fff",color:"#888"}}>ล้าง</button>}
          </div>
          {selTeam ? (
            <>
              <BarChart team={selTeam} customers={customersWithTeam} dark={dark}/>
            ...
            <div style={{height:160,display:"flex",alignItems:"center",justifyContent:"center",color:dark?"#444":"#bbb",fontSize:12}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ค้นหารหัสหรือชื่อลูกค้า..."
          style={{flex:1,minWidth:180,maxWidth:240,padding:"7px 10px",fontSize:12,border:`0.5px solid ${inpBdr}`,borderRadius:8,outline:"none",background:inp,color:txt}}/>
        ...
        <select value={saleFilter} onChange={e=>setSaleFilter(e.target.value)}
          style={{padding:"7px 10px",fontSize:12,border:`0.5px solid ${inpBdr}`,borderRadius:8,background:inp,color:txt,minWidth:120}}>
      ...
      {/* Table */}
      <div style={{background:panel,border:`0.5px solid ${bdr}`,borderRadius:10,overflow:"hidden",marginBottom:14}}>
        <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
          ...
          <tbody>
            {filtered.slice(0,15).map((c,i)=>{
              const bg=c.status==="BLOCK"?(dark?"rgba(80,20,20,0.3)":"rgba(252,235,235,0.25)"):c.status==="WARNING"?(dark?"rgba(80,55,0,0.3)":"rgba(250,238,218,0.25)"):"transparent";
              const tc=TEAM_COLORS[c.team]||"#888";
              return (
                <tr key={c.cust_code} style={{background:bg,borderBottom:`0.5px solid ${dark?"#222":"rgba(0,0,0,0.05)"}`}}>
                  <td style={{padding:"7px 11px",fontSize:11,color:"#aaa"}}>{i+1}</td>
                  <td style={{padding:"7px 11px",fontSize:11,fontWeight:500,color:sub,fontFamily:"monospace"}}>{c.cust_code}</td>
                  <td style={{padding:"7px 11px",fontSize:11,fontWeight:500,color:txt}}>{c.customer_name}</td>
                  ...
                  <td style={{padding:"7px 11px",fontSize:11,color:sub}}>{c.sale}</td>
                  <td style={{padding:"7px 11px",fontSize:11,color:"#aaa"}}>{c.active_br_count}</td>
                  <td style={{padding:"7px 11px",fontSize:11,fontWeight:c.max_days>90?500:400,color:c.max_days>180?"#E24B4A":c.max_days>90?"#EF9F27":txt}}>{c.max_days} วัน</td>
                  <td style={{padding:"7px 11px"}}><StatusBadge status={c.status} dark={dark}/></td>
                  <td style={{padding:"7px 11px",fontSize:10,color:"#aaa"}}>{c.updated_at ? new Date(c.updated_at).toLocaleTimeString("th-TH",{hour:"2-digit",minute:"2-digit"}) : "—"}</td>
                </tr>
              );
            })}
            <tr>
      ...
      <div style={{background:panel,border:`0.5px solid ${bdr}`,borderRadius:10,overflow:"hidden"}}>
        <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
          ...
          <tbody>
            {syncLogs.map((l,i)=>{
              const sc=l.status==="success"?{bg:dark?"#1A2E0A":"#EAF3DE",txt:"#3B6D11",bd:dark?"#3A6014":"#C0DD97",label:"สำเร็จ"}
                      :l.status==="partial"?{bg:dark?"#3D2A00":"#FAEEDA",txt:"#854F0B",bd:dark?"#7A5500":"#FAC775",label:"บางส่วน"}
                      :{bg:dark?"#3D1212":"#FCEBEB",txt:"#A32D2D",bd:dark?"#7A2020":"#F09595",label:"ล้มเหลว"};
              return (
                <tr key={i} style={{borderBottom:`0.5px solid ${dark?"#222":"rgba(0,0,0,0.05)"}`}}>
                  ...
                  <td style={{padding:"7px 11px",fontSize:11,color:sub}}>{l.br_updated||"—"}</td>
                  <td style={{padding:"7px 11px",fontSize:11,color:sub}}>{l.br_closed>0?"-"+l.br_closed:"—"}</td>
                  <td style={{padding:"7px 11px",fontSize:11,color:l.errors>0?"#E24B4A":"#aaa"}}>{l.errors||"—"}</td>
                  <td style={{padding:"7px 11px",fontSize:11,color:"#aaa"}}>{(l.duration_ms/1000).toFixed(1)}</td>
                  <td style={{padding:"7px 11px",fontSize:11,color:"#E24B4A"}}>{l.error_msg||"—"}</td>
                </tr>
              );
            })}
          </tbody>
